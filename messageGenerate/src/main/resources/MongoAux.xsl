...
<xsl:template mode="declareCollections" match="MessagesRoot">
 ...
 <xsl:template mode="createCRUD" match="Message">
    ...
    <xsl:template name="filtersMessage">
        Bson tKeyFilter= Filters.and( <xsl:for-each select="Attribute[@dbKey]">
        <xsl:if test="@type='LocalDate'">
            Filters.eq("<xsl:value-of select='@name'/>", DateUtils.localDateToString(p<xsl:value-of select="../@name"/>.get<xsl:value-of select="functx:capitalize-first(@name)"/>()))</xsl:if>
        <xsl:if test="@type='LocalDateTime'">
            Filters.eq("<xsl:value-of select='@name'/>", DateUtils.localDateTimeToString(p<xsl:value-of select="../@name"/>.get<xsl:value-of select="functx:capitalize-first(@name)"/>()))</xsl:if>
        <xsl:if test="not(@type='LocalDateTime') and not(@type='LocalDate')">
            Filters.eq("<xsl:value-of select='@name'/>", p<xsl:value-of select="../@name"/>.get<xsl:value-of select="functx:capitalize-first(@name)"/>()) </xsl:if><xsl:if test="not(position()=last())">,</xsl:if></xsl:for-each>);
    </xsl:template>
